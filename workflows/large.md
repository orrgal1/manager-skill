...
5. **Review** — one review pass over the whole diff against acceptance, design and risk; the
   reviewer and the rung follow the diff surface (builder.md §7) — `reviewer` on the top rung,
...
9. **Done** — review clean, the set green.
...
- One review pass plus one fix round — under `production` always; under `sprint` when the diff
  surface calls for one — and either way the surface picks the reviewer and the rung (builder.md
  §7).
...
- Review clean: every finding fixed or declined with a reason.
