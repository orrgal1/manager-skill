...
| `quota.limits` | the projection, one entry per limit of that provider: `id`, `used` (fraction), `burn_per_hour`, `projected_at_reset`, `resets_at` (ms), `fits`. Data only — nothing acts on it |
