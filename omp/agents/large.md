...
- One review pass over the whole diff — `reviewer` on the top rung, with `security-reviewer` beside
  it for auth or permissions, else `sweep`: the reviewer and the rung follow the diff surface
  (builder.md §7). Fix every finding or decline it with a reason.
