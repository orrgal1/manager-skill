...
check 'omp was handed the whole role set' 18 "$(jq -r 'length' <<<"$(roles_set)")"
...
check 'setup installs the five size agents' \
  '["large.md","medium.md","plan.md","small.md","tiny.md"]' \
...
check 'the agents are on disk' 5 "$(ls "$agent/agents" | wc -l | tr -d ' ')"

out=$("$PKG" setup --house gemini)
check 'a second setup installs nothing' '[]' "$(jq -c '.agents.installed' <<<"$out")"
check 'a second setup skips all five' \
  '["large.md","medium.md","plan.md","small.md","tiny.md"]' \
...
check '--force reinstalls all five' \
  '["large.md","medium.md","plan.md","small.md","tiny.md"]' \
  "$(jq -c '.agents.installed' <<<"$out")"
