...
~/code/manager-skill/install.sh --omp             # also runs `bin/mgr-package setup`: the six agents (tiny, small, medium, large, plan, sketch) into ~/.omp/agent/agents/ and the house's model package into its config.yml
```
...
| `omp/agents/<name>.md` | The six agent files `mgr setup` installs: `tiny` · `small` · `medium` · `large` · `plan` · `sketch` |
